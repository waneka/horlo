...
next_action: "DEPLOYED to prod (commit 8a49a19 → deployment dpl_ANs3tNYM6YrL4YbYtSySratSbWJa, aliased to www.horlo.app, readyState READY 2026-05-26). AWAITING user prod soft-nav re-test on BOTH routes. CAUTION: the previous reopen was triggered by a MISREAD (hard load mistaken for fixed) — re-test must be a genuine in-app link click (soft nav), not a refresh, on both /u/[username]/[tab] tab-to-tab AND tab/search → /w/[ref]."
