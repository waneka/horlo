...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 01-T1 | 39b-01 | 0 | DISC-11 | — | CTE returns imageUrl on every LineageRow | static + grep | `grep -c "wc.image_url" src/data/hierarchy.ts` ≥ 2 | yes | ✅ green (12e4fc1; count=3) |
| 01-T2 | 39b-01 | 0 | DISC-11 | — | Static guard updated; 1 intentional RED until Plan 39b-05 | static | `npx vitest run tests/static/hierarchy.lineage-3-node.test.ts` (1 expected fail) | yes | ✅ green (acd63e6; 7 pass / 1 intentional RED on getSameFamilyForCatalog assertion — closes in 39b-05) |
| 01-T3 | 39b-01 | 0 | DISC-11 | — | WearEventLite.note exposed | grep | `grep -E "note:?\s*string\s*\|\s*null" src/components/profile/WornCalendar.tsx` ≥ 1 | yes | ✅ green (69394fc; useState/onClick counts unchanged at 2/2) |
...
| 02-T1 | 39b-02 | 1 | DISC-11 | — | Static guard for ReferenceIdentityCard (vacuous-pass pre-file) | static | `npx vitest run tests/static/ReferenceIdentityCard.no-engine.test.ts` exit 0 | yes | ✅ |
| 02-T2 | 39b-02 | 1 | DISC-11 (NSV-06/20) | bundle-leak | Pure renderer; zero engine imports; no 'use client' | static + grep | Same guard non-vacuously green + grep counts 0 for similarity/composer/server-only/'use client' | yes | ✅ |
| 02-T3 | 39b-02 | 1 | DISC-11 (NSV-06/20) | — | Confidence gate + suppression + headline + scale omission | unit | `npx vitest run tests/components/insights/ReferenceIdentityCard.test.tsx` (6 tests green) | yes | ✅ |
| 02-T4 | 39b-02 | 1 | DISC-11 (NSV-06) | — | /watch/{id} mounts card or fallback caption for collection.length === 0 | grep + build | `grep -c "ReferenceIdentityCard" 'src/app/watch/[id]/page.tsx'` ≥ 2; `npm run build` exit 0 | yes | ✅ |
| 02-T5 | 39b-02 | 1 | DISC-11 (NSV-20) | — | /catalog/{id} mounts card or fallback caption (replaces "no card, no CTAs") | grep + build | `grep "Add a few watches to see how this one fits your collection." 'src/app/catalog/[catalogId]/page.tsx'` = 1; build exit 0 | yes | ✅ |
...
| 04-T1 | 39b-04 | 2 | DISC-11 (NSV-18) | T-39b-01, T-39b-04 | Integration test asserts 4 privacy edges + dedup | integration | `set -a; source .env.local; set +a; npx vitest run tests/data/getCollectorsForCatalog.test.ts` (6 tests green when env present) | yes | ✅ green (1d2e4a4; 6 tests authored covering T-39b-01 layers 1+2, T-39b-04, sold-filter A1, ORDER BY DESC, dedup; tests skip via maybe-gate when SUPABASE_SERVICE_ROLE_KEY absent — local .env.local ships publishable/secret on newer Supabase CLI; RED state verified via 11 tsc errors on test file pre-Task-2, resolved to 0 post-DAL) |
| 04-T2 | 39b-04 | 2 | DISC-11 (NSV-18) | T-39b-01, T-39b-04 | Two-layer privacy + self-exclusion + sold-filter at DAL WHERE | grep | `grep -E "profileSettings\.collectionPublic.*true" src/data/discovery.ts` ≥ 2 lines (both queries) | yes | ✅ green (3ec0f4a; collectionPublic.*true count=3, profilePublic.*true count=5, profiles.id != viewerId count=3 fixed-string, inArray(watches.status count=3, count(DISTINCT count=2; both rows query and totalCount query carry identical WHERE clause per Pitfall 4) |
| 04-T3 | 39b-04 | 2 | DISC-11 (NSV-18) | — | Hide-if-empty + count label only when totalCount > 5 | grep | `grep "if (collectors.length === 0) return null" src/components/insights/OtherOwnersRoster.tsx` = 1 + `grep "totalCount > 5" ...` = 1 | yes | ✅ green (c10a2d2; hide-if-empty=1, totalCount > 5=1, size={40}=1 Pitfall 1 A4 substitution, 'use client' count=0 Server Component lock, absolute inset-0=1, focus-visible:ring-2=2) |
| 04-T4 | 39b-04 | 2 | DISC-11 (NSV-18) | — | Mount on /catalog/{id} only (not /watch/{id}) | grep + build | `grep -c "OtherOwnersRoster" 'src/app/watch/[id]/page.tsx'` = 0; `grep -c "OtherOwnersRoster" 'src/app/catalog/[catalogId]/page.tsx'` ≥ 2 | yes | ✅ green (bc557bb; catalog count=3 import+JSX+comment, watch count=0 catalog-only enforced per UI-SPEC line 288, npm run build green Compiled successfully in 6.9s, Rule 1 auto-fix in tests/app/catalog-page.test.ts added vi.mock @/data/discovery with mockGetCollectorsForCatalog default `{ collectors: [], totalCount: 0 }`; net regression delta 0) |
| 05-T1 | 39b-05 | 3 | DISC-11 (NSV-02/16) | — | getSameFamilyForCatalog exported (closes RED from 01-T2) | grep + static | `grep -E "export (async )?function getSameFamilyForCatalog" src/data/hierarchy.ts` = 1; `npx vitest run tests/static/hierarchy.lineage-3-node.test.ts` exit 0 (was RED in 01-T2; now GREEN) | yes | ✅ green (1a6f3fa; export count=1, lineage-3-node 8/8 pass, intentional RED closed; live COUNT + two-pass + hide-if-empty + LIMIT 6 default all asserted) |
| 05-T2 | 39b-05 | 3 | DISC-11 (NSV-02/16) | — | Both rails ship as Server Components; hide-if-empty + 6-cap + typography lock | grep | `grep -c "if (rows.length === 0) return null" src/components/insights/SameFamilyRail.tsx src/components/insights/LineageRail.tsx` = 2; `grep -c "font-semibold" SameFamilyRail.tsx LineageRail.tsx` = 0 | yes | ✅ green-with-deviation (2978758; hide-if-empty count=2, no 'use client', .slice(0,6) cap on LineageRail, all 5 RELATIONSHIP_LABELS present, "1 collector" singular + "{N} collectors" plural; **Rule 1 auto-fix:** font-medium→font-semibold lint flip — UI-SPEC drafted font-medium but tests/no-raw-palette.test.ts forbids it; same flip as 39b-02 c205617 / 39b-03 049b3f4; plan AC "grep -c font-semibold = 0" superseded by project lint enforcement) |
| 05-T3 | 39b-05 | 3 | DISC-11 (NSV-02/16) | — | /watch/{id} mounts both rails for all viewer states | grep + build | `grep -c "SameFamilyRail" 'src/app/watch/[id]/page.tsx'` ≥ 2; `grep -c "LineageRail" ...` ≥ 2; build exit 0 | yes | ✅ green (ae1d737; SameFamilyRail count=2, LineageRail count=2, watch.catalogId? guard present, OtherOwnersRoster count=0 catalog-only enforced, B1 invariant grep on WatchDetail.tsx=0, page.tsx remains Server Component, npm run build green) |
| 05-T4 | 39b-05 | 3 | DISC-11 (NSV-02/16) | — | /catalog/{id} mounts both rails after OtherOwnersRoster | grep + build | `grep -c "SameFamilyRail" 'src/app/catalog/[catalogId]/page.tsx'` ≥ 2; render-order JSX snippet captured in SUMMARY | yes | ✅ green-with-deviation (b681506; SameFamilyRail count=2 import+JSX, LineageRail count=3 import+JSX+comment, OtherOwnersRoster preserved count=4, render order verdict→roster→SameFamily→Lineage→CTAs asserted in SUMMARY; **Rule 1 auto-fix:** vi.mock('@/data/hierarchy') added to tests/app/catalog-page.test.ts — same vi.mock pattern as 39b-04 bc557bb; net regression delta -1 / project baseline 52→51 failed as intentional RED from 01-T2 closed) |
