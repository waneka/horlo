...
| DISC-AUDIT-21 | / | WywtRail self-placeholder tile (opens WywtPostDialog) | — | Live | src/components/home/WywtTile.tsx:56 onClick={onOpenPicker}; src/components/home/WywtRail.tsx:111 setPickerOpen(true) — own-collection picker affordance | owner-populated | both |
| DISC-AUDIT-22 | / | WywtRail standard tile (opens WywtOverlay at index) | — | Live | src/components/home/WywtTile.tsx:85 onClick={onOpen}; src/components/home/WywtRail.tsx:108 openAt(tile) — renders against viewer's own + follows' wear-event tiles; affordance INTENDED for populated | owner-populated | both |
| DISC-AUDIT-23 | / | WywtOverlay close button | — | Live | src/components/home/WywtOverlay.tsx:96 — onClick onOpenChange(false); modal control gate-independent | N/A | both |
| DISC-AUDIT-24 | / | WywtOverlay previous slide chevron | — | Live | src/components/home/WywtOverlay.tsx:105 — emblaApi?.scrollPrev(); modal control gate-independent | N/A | desktop |
| DISC-AUDIT-25 | / | WywtOverlay next slide chevron | — | Live | src/components/home/WywtOverlay.tsx:114 — emblaApi?.scrollNext(); modal control gate-independent | N/A | desktop |
| DISC-AUDIT-26 | / | WywtSlide brand+model Link (in overlay) | /watch/{watchId} | Live | src/components/home/WywtSlide.tsx:77; affordance INTENDED for populated (fresh-account WywtRail typically empty per WYWT-10 visibility filter) | owner-populated | both |
| DISC-AUDIT-27 | / | WywtSlide Add to wishlist button (in overlay) | addToWishlistFromWearEvent Server Action | Live | src/components/home/WywtSlide.tsx:110 onClick={handleAddToWishlist}; rendered against follows' wear-event slides | owner-populated | both |
| DISC-AUDIT-28 | / | WywtSlide Retry button (error state) | addToWishlistFromWearEvent Server Action | Live | src/components/home/WywtSlide.tsx:101 onClick={handleAddToWishlist}; same overlay context as DISC-AUDIT-27 | owner-populated | both |
| DISC-AUDIT-29 | / | CollectorsLikeYou RecommendationCard Link | /watch/{representativeWatchId} | Live | src/components/home/RecommendationCard.tsx:21; rec engine requires collection signal (gate-suppressed for empty-collection viewer) | owner-populated | both |
| DISC-AUDIT-30 | / | NetworkActivityFeed ActivityRow whole-row Link | /u/{username}/collection | Live | src/components/home/ActivityRow.tsx:34 — absolute-inset Link overlay; activity feed requires follows | owner-populated | both |
| DISC-AUDIT-31 | / | NetworkActivityFeed ActivityRow watch-name Link (nested) | /watch/{watchId} | Live | src/components/home/ActivityRow.tsx:50 — nested z-10 Link; same activity-feed gate as DISC-AUDIT-30 | owner-populated | both |
| DISC-AUDIT-32 | / | NetworkActivityFeed AggregatedActivityRow whole-row Link | /u/{username}/collection | Live | src/components/home/AggregatedActivityRow.tsx:36; same activity-feed gate as DISC-AUDIT-30 | owner-populated | both |
| DISC-AUDIT-33 | / | NetworkActivityFeed Load more button | loadMoreFeed Server Action | Live | src/components/home/LoadMoreButton.tsx:78 onClick={handleClick}; only renders when feed has additional pages (owner-populated) | owner-populated | both |
| DISC-AUDIT-34 | / | FeedEmptyState "Find collectors to follow" CTA anchor | #suggested-collectors (same-page anchor) | Live | src/components/home/FeedEmptyState.tsx:21 — Link href="#suggested-collectors"; renders when feed has zero activity (fresh-account branch — owner-populated has feed activity from follows) | fresh-account | both |
| DISC-AUDIT-35 | / | PersonalInsightsGrid SleepingBeautyCard Link | /watch/{watchId} | Live | src/components/home/SleepingBeautyCard.tsx:32; PersonalInsightsGrid hidden on empty collection per L-01 I-04 | owner-populated | both |
| DISC-AUDIT-36 | / | PersonalInsightsGrid MostWornThisMonthCard Link | /watch/{watchId} | Live | src/components/home/MostWornThisMonthCard.tsx:20; PersonalInsightsGrid I-04 gate | owner-populated | both |
| DISC-AUDIT-37 | / | PersonalInsightsGrid WishlistGapCard Link | /u/me/wishlist?filter={role} | Live | src/components/home/WishlistGapCard.tsx:23; PersonalInsightsGrid I-04 gate | owner-populated | both |
| DISC-AUDIT-38 | / | PersonalInsightsGrid CommonGroundFollowerCard Link | /u/{username}/common-ground | Live | src/components/home/CommonGroundFollowerCard.tsx:26; requires both follows AND collection overlap | owner-populated | both |
| DISC-AUDIT-39 | / | SuggestedCollectors SuggestedCollectorRow whole-row Link | /u/{username}/collection | Live | src/components/home/SuggestedCollectorRow.tsx:36 — absolute-inset Link overlay; rail renders for any authenticated viewer; affordance INTENDED for populated case (per `<interfaces>` collapse rule) | owner-populated | both |
| DISC-AUDIT-40 | / | SuggestedCollectors SuggestedCollectorRow FollowButton | followUser Server Action (or /login?next= unauth) | Live | src/components/home/SuggestedCollectorRow.tsx:87 + src/components/profile/FollowButton.tsx:132 onClick={handleClick}; same rail visibility as DISC-AUDIT-39 | owner-populated | both |
| DISC-AUDIT-41 | / | SuggestedCollectors Load more button | loadMoreSuggestions Server Action | Live | src/components/home/LoadMoreSuggestionsButton.tsx:79 onClick={handleClick}; rail visibility same as DISC-AUDIT-39 | owner-populated | both |
| DISC-AUDIT-42 | / | WatchPickerDialog list-item button (select watch) | — | Live | src/components/home/WatchPickerDialog.tsx:198 onClick={...setSelectedId}; dialog opened by owner taking action on own collection | owner-populated | both |
| DISC-AUDIT-43 | / | WatchPickerDialog Keep browsing dismiss button | — | Live | src/components/home/WatchPickerDialog.tsx:235 onClick={handleDismiss}; same dialog context as DISC-AUDIT-42 | owner-populated | both |
| DISC-AUDIT-44 | / | WatchPickerDialog Log wear submit button | markAsWorn Server Action | Live | src/components/home/WatchPickerDialog.tsx:242 onClick={handleSubmit}; markAsWorn fires on viewer's own watch | owner-populated | both |
| DISC-AUDIT-45 | / | WatchPickerDialog empty-state Add watch Link | /watch/new?returnTo=... | Live | src/components/home/WatchPickerDialog.tsx:154 — Link in empty-collection branch; dialog branches to empty-state when viewer has zero owned watches (fresh-account state) | fresh-account | both |
| DISC-AUDIT-46 | / | WatchPickerDialog empty-state Keep browsing button | — | Live | src/components/home/WatchPickerDialog.tsx:150 onClick={handleDismiss}; same empty-collection branch as DISC-AUDIT-45 | fresh-account | both |
| DISC-AUDIT-47 | /explore | ExploreHero "Browse popular collectors" CTA Link | /explore/collectors | Live | src/components/explore/ExploreHero.tsx:28; gated at src/app/explore/page.tsx:38 by `followingCount < 3 && wearEventsCount < 1` (G-1) — only fresh-account satisfies; owner-populated state suppresses ExploreHero entirely | fresh-account | both |
| DISC-AUDIT-48 | /explore | PopularCollectors "See all" header Link | /explore/collectors | Live | src/components/explore/PopularCollectors.tsx:36; rail renders for any authenticated viewer; affordance INTENDED for populated case | owner-populated | both |
| DISC-AUDIT-49 | /explore | PopularCollectorRow whole-row Link | /u/{username}/collection | Live | src/components/explore/PopularCollectorRow.tsx:42 — absolute-inset Link; same rail visibility as DISC-AUDIT-48 | owner-populated | both |
| DISC-AUDIT-50 | /explore | PopularCollectorRow FollowButton | followUser Server Action (or /login?next=) | Live | src/components/explore/PopularCollectorRow.tsx:61 + src/components/profile/FollowButton.tsx:132; same rail visibility as DISC-AUDIT-48 | owner-populated | both |
| DISC-AUDIT-51 | /explore | TrendingWatches "See all" header Link | /explore/watches | Live | src/components/explore/TrendingWatches.tsx:32; rail visibility same as DISC-AUDIT-48 | owner-populated | both |
| DISC-AUDIT-52 | /explore | TrendingWatches DiscoveryWatchCard Link | /catalog/{catalogId} | Live | src/components/explore/DiscoveryWatchCard.tsx:29; rail visibility same as DISC-AUDIT-48 | owner-populated | both |
| DISC-AUDIT-53 | /explore | GainingTractionWatches "See all" header Link | /explore/watches | Redundant | src/components/explore/GainingTractionWatches.tsx:35; Redundant to DISC-AUDIT-51 — both rails point at the same /explore/watches sub-route | owner-populated | both |
| DISC-AUDIT-54 | /explore | GainingTractionWatches DiscoveryWatchCard Link | /catalog/{catalogId} | Live | src/components/explore/DiscoveryWatchCard.tsx:29 — same component as TrendingWatches; rail visibility same as DISC-AUDIT-48 | owner-populated | both |
| DISC-AUDIT-55 | /explore | /explore/collectors paginated 'see all' page (D-06 summary row) | /explore/collectors | Live | src/app/explore/collectors/page.tsx:23 — paginated 'see all' list (LIMIT 50, no pagination); downstream affordances identical to parent rail rows DISC-AUDIT-49..50 | owner-populated | both |
| DISC-AUDIT-56 | /explore | /explore/watches paginated 'see all' page (D-06 summary row) | /explore/watches | Live | src/app/explore/watches/page.tsx:56 — paginated 'see all' list (Trending + Gaining Traction stacked sections, LIMIT 50 each); downstream affordances identical to parent rail rows DISC-AUDIT-52, DISC-AUDIT-54 | owner-populated | both |
...
| DISC-AUDIT-60 | /search | PeopleSearchRow whole-row Link | /u/{username}/collection | Live | src/components/search/PeopleSearchRow.tsx:47 — absolute-inset Link; results render for any authenticated viewer; affordance INTENDED for populated | owner-populated | both |
| DISC-AUDIT-61 | /search | PeopleSearchRow FollowButton | followUser Server Action (or /login?next=) | Live | src/components/search/PeopleSearchRow.tsx:102 + src/components/profile/FollowButton.tsx:132; same results rendering as DISC-AUDIT-60 | owner-populated | both |
| DISC-AUDIT-62 | /search | WatchSearchRow body Link (image + brand/model + Owned/Wishlist pill) | /catalog/{catalogId} | Live | src/components/search/WatchSearchRow.tsx:30; Owned/Wishlist pill is viewer-state-dependent data inside (per-viewer collection state) but the Link affordance EXISTS for any viewer | owner-populated | both |
| DISC-AUDIT-63 | /search | WatchSearchRowsAccordion chevron trigger (expand verdict) | getVerdictForCatalogWatch Server Action | Live | src/components/search/WatchSearchRowsAccordion.tsx:147 — Accordion.Trigger; src/components/search/WatchSearchRowsAccordion.tsx:58 lazy-fetch verdict; expand affordance gate-independent but produces verdict (which is owner-populated-only — see DISC-AUDIT-64) | owner-populated | both |
| DISC-AUDIT-64 | /search | WatchSearchRowsAccordion Verdict pill/CollectionFitCard render (in panel) | — | Live | src/components/search/WatchSearchRowsAccordion.tsx:160 + src/components/insights/CollectionFitCard.tsx:41 — Verdict Badge rendered when verdict !== null; verdict suppressed for empty-collection viewer (G-4-equivalent client-side gate; computeVerdictBundle only fires when collection.length>0) | owner-populated | both |
| DISC-AUDIT-65 | /search | WatchSearchRowsAccordion "Add to Wishlist" button (in panel) | addWatch Server Action + toast View → /u/{viewer-username}/wishlist | Live | src/components/search/WatchSearchRowsAccordion.tsx:169 onClick={handleAddToWishlist}; src/components/search/WatchSearchRowsAccordion.tsx:105 router.push wishlist; renders for any viewer (button is gate-independent of collection.length) | N/A | both |
| DISC-AUDIT-66 | /search | WatchSearchRowsAccordion "Add to Collection" button (in panel) | /watch/new?catalogId=...&intent=owned&returnTo=... | Live | src/components/search/WatchSearchRowsAccordion.tsx:189 onClick={handleAddToCollection}; src/components/search/WatchSearchRowsAccordion.tsx:125 router.push; renders for any viewer | N/A | both |
| DISC-AUDIT-67 | /search | CollectionSearchRow whole-row Link | /u/{username}/collection | Live | src/components/search/CollectionSearchRow.tsx:47 — absolute-inset Link; results render for any authenticated viewer | owner-populated | both |
| DISC-AUDIT-68 | /search | SuggestedCollectorsForSearch SuggestedCollectorRow whole-row Link (pre-query / no-results state) | /u/{username}/collection | Live | src/app/search/page.tsx:88 → src/components/home/SuggestedCollectorRow.tsx:36 (rendered inside SearchPageClient children slot); pre-query rail aimed at low-follow viewers but renders for both | owner-populated | both |
| DISC-AUDIT-69 | /search | SuggestedCollectorsForSearch FollowButton (pre-query / no-results state) | followUser Server Action | Live | src/components/home/SuggestedCollectorRow.tsx:87 + src/components/profile/FollowButton.tsx:132; same rail as DISC-AUDIT-68 | owner-populated | both |
| DISC-AUDIT-70 | /catalog/{catalogId} | Verdict label/pill on CollectionFitCard (cross-user framing) | — | Live | src/app/catalog/[catalogId]/page.tsx:145 + src/components/insights/CollectionFitCard.tsx:41 — Verdict Badge rendered when collection.length > 0 (G-4 owner-populated branch) | owner-populated | both |
| DISC-AUDIT-71 | /catalog/{catalogId} | Verdict mostSimilar list watch row (text-only, no link) | — | Live | src/components/insights/CollectionFitCard.tsx:69 — non-link list item; rendered inside the verdict block (same G-4 owner-populated gate as DISC-AUDIT-70); SEED-004 Rdio violation candidate (no click affordance to walk to similar watch) — left as Live for source pass; Wave 3 may downgrade | owner-populated | both |
| DISC-AUDIT-72 | /catalog/{catalogId} | "You own this" callout Link (self-via-cross-user framing G-3) | /watch/{viewer-owned-id} | Live | src/components/insights/CollectionFitCard.tsx:114 — `ownerHref` Link; G-3 fires when viewer owns catalog ref (src/app/catalog/[catalogId]/page.tsx:70-78); only owner-populated state can hit this branch (requires owning a watch derived from the catalog ref) | owner-populated | both |
| DISC-AUDIT-73 | /catalog/{catalogId} | CatalogPageActions: Add to Wishlist CTA (cross-user framing, collection>0) | addWatch Server Action + toast View → /u/{viewer-username}/wishlist | Live | src/app/catalog/[catalogId]/page.tsx:151 → src/components/watch/CatalogPageActions.tsx:147 onClick={handleWishlist}; CatalogPageActions.tsx:106 router.push wishlist; G-4 owner-populated branch (collection.length > 0 + cross-user framing) | owner-populated | both |
| DISC-AUDIT-74 | /catalog/{catalogId} | CatalogPageActions: Add to Collection CTA | /watch/new?catalogId=...&intent=owned&returnTo=... | Live | src/components/watch/CatalogPageActions.tsx:164 onClick={handleCollection}; CatalogPageActions.tsx:128 router.push; G-4 owner-populated branch | owner-populated | both |
| DISC-AUDIT-75 | /catalog/{catalogId} | CatalogPageActions: Skip CTA | router.back() | Live | src/components/watch/CatalogPageActions.tsx:174 onClick={handleSkip}; CatalogPageActions.tsx:135 router.back(); G-4 owner-populated branch | owner-populated | both |
| DISC-AUDIT-130 | /catalog/{catalogId} | Verdict + CatalogPageActions absent (G-4 fresh-account suppression branch) | — | Missing | src/app/catalog/[catalogId]/page.tsx:79-113 — `else: collection.length === 0 → verdict stays null AND actionsSpec stays null → no card, no CTAs (D-05 + D-07 empty-collection rule)`; SEED-004 Rdio violation: fresh-account viewer lands on /catalog/{id} with header + image only — no verdict, no walk-forward affordance to wishlist/collection, no walk-back to other watches in the same family. Counterpart of owner-populated rows DISC-AUDIT-70, 73, 74, 75. | fresh-account | both |
| DISC-AUDIT-76 | /watch/{id} | WatchDetail Mark as Worn button (owner-only, owned status) | markAsWorn Server Action | Live | src/components/watch/WatchDetail.tsx:189 onClick={handleMarkAsWorn} — gated `viewerCanEdit && watch.status === 'owned'`; G-7 same-user framing only (viewerCanEdit=isOwner per src/app/watch/[id]/page.tsx:63) | owner-populated | both |
| DISC-AUDIT-77 | /watch/{id} | Edit Link on WatchDetail (owner-only) | /watch/{id}/edit | Live | src/components/watch/WatchDetail.tsx:195 — gated `viewerCanEdit` (G-7 same-user framing only); affordance literally absent in cross-user framing | owner-populated | both |
| DISC-AUDIT-78 | /watch/{id} | Delete dialog trigger on WatchDetail (owner-only) | — | Live | src/components/watch/WatchDetail.tsx:199 — DialogTrigger Button gated `viewerCanEdit` (G-7 same-user framing); absent in cross-user | owner-populated | both |
| DISC-AUDIT-79 | /watch/{id} | Delete confirm button on WatchDetail (owner-only, in dialog) | removeWatch Server Action → router.push('/') | Live | src/components/watch/WatchDetail.tsx:220 onClick={handleDelete}; WatchDetail.tsx:84 router.push('/'); G-7 same-user framing only | owner-populated | both |
| DISC-AUDIT-80 | /watch/{id} | Delete dialog Cancel button on WatchDetail | — | Live | src/components/watch/WatchDetail.tsx:213 onClick={() => setIsDeleteDialogOpen(false)}; rendered inside owner-only Delete dialog (DISC-AUDIT-78); G-7 same-user framing only | owner-populated | both |
| DISC-AUDIT-81 | /watch/{id} | WatchDetail Verdict label/pill on CollectionFitCard | — | Live | src/components/watch/WatchDetail.tsx:444 + src/components/insights/CollectionFitCard.tsx:41 — Verdict Badge rendered when verdict !== null (G-6 owner-populated branch — collection.length > 0 gate at src/app/watch/[id]/page.tsx:41); renders for both same-user and cross-user framings as long as viewer has populated collection | owner-populated | both |
| DISC-AUDIT-82 | /watch/{id} | WatchDetail Verdict mostSimilar list watch row (text-only, no link) | — | Live | src/components/insights/CollectionFitCard.tsx:69 — text-only; rendered inside verdict block (same G-6 owner-populated gate as DISC-AUDIT-81); SEED-004 Rdio violation candidate (no click affordance to similar watch) — Live for source pass; Wave 3 may downgrade | owner-populated | both |
| DISC-AUDIT-83 | /watch/{id} | WatchDetail Flag-as-good-deal Checkbox (owner-only, wishlist/grail) | editWatch Server Action | Live | src/components/watch/WatchDetail.tsx:172 onCheckedChange (gated `isWishlistLike && viewerCanEdit`); G-7 same-user framing only | owner-populated | both |
| DISC-AUDIT-131 | /watch/{id} | Verdict / CollectionFitCard absent (G-6 fresh-account suppression branch) | — | Missing | src/app/watch/[id]/page.tsx:41 — `if (collection.length > 0)` branch; else verdict stays null and CollectionFitCard does not render. SEED-004 Rdio violation: fresh-account viewer lands on /watch/{id} with watch detail but no verdict/fit context — no walk-forward affordance to similar watches via the verdict mostSimilar list (DISC-AUDIT-82 counterpart absent). Counterpart of owner-populated rows DISC-AUDIT-81, 82. | fresh-account | both |
