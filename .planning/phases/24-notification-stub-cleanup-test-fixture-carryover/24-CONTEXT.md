...
- **D-01 (Claude's Discretion, REVISED 2026-05-01 per RESEARCH.md): Defense-in-depth — both a standalone pre-migration script AND an embedded SQL `DO $$ … RAISE EXCEPTION` block at the top of the rename+recreate migration. Use a WHITELIST predicate, not a blacklist.**
  - **Whitelist phrasing (revised):** `WHERE type::text NOT IN ('follow','watch_overlap')`. Same coverage as the original blacklist `IN ('price_drop','trending_collector')` for the known stub values, PLUS catches any unexpected/corrupt values that may have slipped in. Strict improvement; chosen per researcher recommendation.
  - **Standalone script:** `scripts/preflight-notification-types.ts` (invokable via `npm run db:preflight-notification-cleanup`) connects to the configured `DATABASE_URL`, runs `SELECT count(*) FROM notifications WHERE type::text NOT IN ('follow','watch_overlap')`, and exits non-zero if the count is non-zero. Run in CI before deploying the migration.
  - **In-migration assertion:** A `DO $$ DECLARE n int; BEGIN SELECT count(*) INTO n FROM notifications WHERE type::text NOT IN ('follow','watch_overlap'); IF n > 0 THEN RAISE EXCEPTION 'preflight failed: % rows hold values outside the new enum domain', n; END IF; END $$;` block as the first statement in the migration SQL.
...
- **D-02 (Claude's Discretion): Standard representative coverage — every public behavior with happy path + 1-2 representative edge cases per behavior.** Not minimum-viable smoke; not exhaustive table-driven. Rationale: these suites have been deferred since v1.0 — a token check-box would re-defer the debt; comprehensive table-driven explodes phase scope. Standard representative is the level of rigor v3.0 phases (Phase 11/13/15 component & integration tests) actually shipped.
  ...
  **Per-suite targets** (planner refines specifics):
  ...
  - **TEST-04 — `tests/store/watchStore.test.ts` (REVISED 2026-05-01 per RESEARCH.md):** `beforeEach` resets store via `useWatchStore.setState(initialState, true)` (Zustand v5 replace-mode). Cover ONLY the surface that exists today: `setFilter` (slice = status / style / role / dial color — set, unset, multi-select within a slice) and `resetFilters` (clears every slice back to initial). The store has NO `addWatch` / `updateWatch` / `deleteWatch` actions and NO `getFilteredWatches()` selector in the current code — those were aspirational targets in the original CONTEXT.md draft. If the planner finds them, treat as a refactor signal and pause; do not invent fixtures. Excludes integration with persistence middleware (out of scope for unit tests).

  - **TEST-05 — `tests/api/extract-watch.test.ts` (REVISED 2026-05-01 per RESEARCH.md)** (separate from existing `extract-watch-auth.test.ts` which only covers auth gate): Happy path (valid URL → structured-data success), categorized failures (host-403, structured-data-missing, LLM-timeout, generic-network), URL validation (non-http(s) protocol rejected via the manual protocol allow-list in the route, NOT Zod). The route uses manual input checks today; **defer Zod adoption** — do NOT introduce a Zod schema in TEST-05's name. If Zod becomes desirable, route it through a future polish phase. Mock outbound `fetch` via vitest's `vi.mock` matching the pattern in existing `tests/api/` and `tests/data/` integration tests — confirm pattern by reading `tests/setup.ts` and `tests/api/extract-watch-auth.test.ts`.
