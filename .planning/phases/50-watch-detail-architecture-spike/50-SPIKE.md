...
**Primary recommendation: Variant B — URL canonicalization**

Variant B is the correct call for v5.2. It scores the highest total (28) in the §6 Decision Matrix, is the only variant that simultaneously scores 5 on both cost criteria (zero entry-point rewrites, 1-2 files touched), and directly retires the Phase 48 BUG-01 maintenance tax — the concrete, in-production evidence that the two-route split carries ongoing carrying cost.

### 8.1 Rationale

**Why Variant B.** The §6 matrix is decisive: Variant B scores 3/4/4/3/5/5/4, totaling 28 — the highest sum in the set. Variant A (keep separate, 27 total) is the only variant within one point of Variant B, and the comparison exposes why the tie-breaking v7.0 lens evidence favors B over A. The single criterion where B and A diverge in B's favor is per-user data shape (B: 4 vs A: 3), driven directly by the Phase 48 BUG-01 retirement evidence in §4.B: the `status='owned'` fix at `src/app/catalog/[catalogId]/page.tsx:296` is not the last word on `findViewerWatchByCatalogId` — every new `WatchStatus` value, every new wishlist edge case, every new "viewer has a special relationship to this ref" scenario requires another edit to the in-route framing flip at lines 107–115. Variant B eliminates the flip entirely by replacing it with a server-level `redirect()` from `next/navigation`, which is the correct Next.js 16 App Router canonicalization API (as confirmed by `node_modules/next/dist/docs/01-app/02-guides/redirecting.md:41`). Variant A preserves the flip and its tax intact.

On UX clarity, Variant B scores 3 vs A's 2. The §2 matrix documents the structural root: the owner cell on `/catalog/[catalogId]` today delivers a framing-flipped callout ("You own this watch") with a link back to the canonical per-user URL — a non-canonical surface with redirect-flavored inline rendering, not a clear owner landing. Variant B replaces that construct with an actual redirect, producing a clean single-meaning owner surface at `/watch/[id]`.

The v7.0 lens (§5.B) gives Variant B a carousel fit score of 3. Two render sites survive: owners are redirected to `/watch/[id]` (single write-capable carousel site), but non-owner viewers continue to receive the catalog route's read-only carousel. This is a material gap vs Variant C (which scores 5 on v7.0 carousel fit by collapsing all viewer-state cells to one route). The gap is real — but Variant C costs 1/1 on both cost criteria (19 entry-point rewrites, 19+ files touched), making it the architecturally superior but practically expensive option for v5.2. The §5.C lens establishes that Variant C is the correct v7.0 target architecture once the multi-photo model forces the carousel surface question. The relationship is sequential, not competing: ship Variant B in v5.2 to retire the BUG-01 maintenance tax and establish the owner redirect as the canonical pattern; revisit for Variant C at the v7.0 carousel implementation phase.

All five variants in A-E preserve the two-layer privacy gate in `getWatchByIdForViewer` (`src/data/watches.ts:193`): the outer RLS + WHERE-inner gate requiring `profile_public=true` AND per-tab visibility fires on all cross-user reads regardless of which route shell is active. Variant B changes only which route an owner-viewer lands on; it does not relax or restructure the privacy gate.

**Why not the other variants.** Variant A (27) — the §6 matrix score is one point lower than Variant B, and the delta is fully explained by the per-user data shape score (3 vs 4): Variant A keeps the in-route framing flip and its BUG-01 maintenance tax intact. The §4.A summary names this explicitly: "the in-route D-08 framing flip (`src/app/catalog/[catalogId]/page.tsx:107-115`) and its maintenance tax survive untouched." There is no architectural benefit to Variant A that the matrix captures; its high scores are entirely on stability/cost/reversibility criteria that represent choosing not to act. Variant C (23) — maximum disruption cost disqualifies it for v5.2: entry-point disruption score 1 (all 19 entry points rewrite, §3.2 full list) and migration cost score 1 (19+ files touched including new route + 2 old route shells). Variant C is the v7.0 target, not the v5.2 action. Variant D (23) — absorbing `/watch/[id]` into `/catalog/[catalogId]` requires 12 entry-point rewrites (score 2) and 13-14 files touched (score 2), plus an open UI-SPEC decision on `OtherOwnersRoster` visibility for owners (§4.D forcing-function note) that Phase 50 cannot pre-decide. Variant E (21) — the UUID-dispatch fragility on a route that currently carries clean per-user semantics is a code-clarity cost not captured in the simple file count: `getWatchByIdForViewer` try → `getCatalogById` fallback adds a non-trivial branching surface (§4.E forcing-function note), and the smallest-blast-radius of the full-merge variants (7 rewrites) still dwarfs Variant B's zero rewrites.

**What Variant B solves.** Variant B retires the Phase 48 BUG-01 bug class in full. The fix is not a status-filter patch (which is what line 296 provides today) — it is removal of the in-route framing flip at `src/app/catalog/[catalogId]/page.tsx:107-115` entirely. Once the flip code is gone, `findViewerWatchByCatalogId` no longer needs status-scoped ownership detection because the flip it serves no longer exists. The wishlist-holder and sold-this cells in §2's matrix (the exact edge cases that produced BUG-01 in Phase 48) become irrelevant to the catalog route's rendering — those viewers simply receive the catalog cross-user view without any framing override. The implementation is a 1-2 file change: add `redirect(\`/watch/${viewerOwnedRow.id}\`)` at `src/app/catalog/[catalogId]/page.tsx` (imported from `next/navigation`, invoked after `findViewerWatchByCatalogId` returns non-null) and remove the VerdictBundleSelfOwned construction at lines 107–115. One test file covers the redirect assertion for owner viewers.

**What Variant B exposes for v6.0 and v7.0.** The §5.B lens establishes the forward trajectory. For v6.0 social (SEED-012), Variant B means the likes/comments surface for owner wear-entries lives in the `WatchDetail` client island (`src/components/watch/WatchDetail.tsx`, `'use client'`) on `/watch/[id]` — the owner is always on the per-user route. Cross-user likes/comments on the catalog route remain separate from the owner's write surface, which is a clean separation v6.0's social layer can exploit. For v7.0 Watch Photos (SEED-013), the owner-facing carousel (with the add-photo affordance) has a single render site — the `WatchDetail` island on `/watch/[id]`. The catalog route still needs its own read-only carousel for non-owner viewers, meaning v7.0 will implement two carousel compositions. The §5.C lens documents why Variant C eliminates that duplication at v7.0 cost — the Phase 50.1 implementation of Variant B should carry a `TODO: revisit for Variant C in v7.0` comment at the carousel composition site so the v7.0 milestone inherits an explicit decision anchor rather than a surprise.
