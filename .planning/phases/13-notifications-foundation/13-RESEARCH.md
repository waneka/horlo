...
1. **Status filter on watch-overlap matching — owned only, or owned + grail?**
...
2. **When exactly to update `notifications_last_seen_at` during the `/notifications` page visit.**
...
3. **Payload shape for `follow` notifications — include actor avatar at write time?**
...
4. **Temporary Phase-14 bell placement — in `Header.tsx` or somewhere else?**
...
5. **Should `logNotification` pre-resolve the actor's username, or accept ids only and denormalize internally?**
   ...
   - Recommendation: Denormalize inside `logNotification`. Callers pass `actorUserId` + `recipientUserId` + type-specific extra fields (watch id/brand/model for overlap). The logger looks up the actor's profile once before inserting. Latency is in the non-blocking fire-and-forget path, so user-facing mutation is unaffected. Document this decision in the logger module.
