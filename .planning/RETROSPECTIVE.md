...
**Shipped:** 2026-06-10 (code-complete 2026-05-30)
**Phases:** 1 (75) | **Plans:** 2 | **Tasks:** 6
**Source:** SEED-017 — operator observation that the home "From Collectors Like You" rail surfaces the same ~2 watches across collection changes despite being the largest above-the-fold content slot

### What Was Built

The smallest milestone since v1.0 — a single phase / 2 wave-1-parallel plans targeting the home recs rail's freshness in two dimensions:

- **Phase 75 Plan 01 — DISC-RECS-CACHE (cache-tag invalidation)**: `CollectorsLikeYou.tsx` registers `cacheTag('viewer:${viewerId}:recs')` inside its `'use cache'` scope (Pitfall 7 cross-user safety preserved — viewerId stays in cache key). All four watch-mutation Server Actions (`addWatch`, `editWatch`, `removeWatch`, `moveWishlistToCollection`) call `updateTag('viewer:${user.id}:recs')` immediately after their existing `revalidatePath('/')`. Read-your-own-writes semantics — the user who just mutated sees the rec change THIS render, not after `cacheLife('minutes')` expires. Mirrors v7.0 Phase 63 `viewer:${id}:counts` tag shape.

- **Phase 75 Plan 02 — DISC-RECS-VARIATION (algorithm rotation + sparse-pool top-up)**: `SEED_POOL_SIZE` 15→30 in `src/data/recommendations.ts`. New `ROTATION_WINDOW_MS = 6h`. Exported `seedFor` (djb2 hash of `(viewerId, floor(Date.now() / 6h))`) + inline `mulberry32` 5-LOC PRNG (no external dep). Fisher-Yates shuffle of top-30 → first 15. Same window = same shuffle = cache-stable; next window = different shuffle = rail rotates 4× daily. Sparse-pool fallback `topUpFromCatalogPopularity()` fires when `candidateMap.size < 8` and pulls from `watches_catalog` ordered by `(ownersCount DESC, brand ASC) LIMIT 20`; synthetic rows emit `representativeOwnerId: null` (type widened, non-breaking) and route through existing community-fallback rationale — no new copy. The rail never looks broken with 1-2 cards on fresh small user bases.

~2 hours of code work on 2026-05-30 (14 commits, +874/−11 LOC across 6 files, 2 new test files at 228 + 413 LOC). Close held open until 2026-06-09; the 10-day gap exercised the DISC-RECS-VARIATION ≥6h rotation window organically without operator effort.

### What Worked

- **Wave-1-parallel plan split with zero file overlap** — Plan 01 touched `CollectorsLikeYou.tsx` + `src/app/actions/watches.ts`; Plan 02 touched `src/data/recommendations.ts` + `src/lib/discoveryTypes.ts`. No conflicts; both plans verified independently. The 2-plan polish-milestone shape is now the smallest viable GSD-cycle granularity (smaller than v8.1's 3 phases).
- **Executor-detected API drift** — Plan 01 spec'd legacy single-arg `revalidateTag(tag)`. Executor caught the Next 16 deprecation, migrated all 4 sites + test assertions to `updateTag(tag)`, documented in commit + SUMMARY. Plan stayed valid (D-02 semantic preserved); only the syntax shifted. This is the right kind of executor judgment to trust.
- **Single-window deterministic PRNG without an external dep** — `mulberry32` is 5 LOC inline (no `seedrandom`, no `chance`, no library overhead). Cache-stable within a 6h window because the seed is `(viewerId, floor(Date.now() / 6h))` — every request within the window gets the same shuffle order, so the `'use cache'` hit rate isn't degraded by the rotation logic.
- **Non-breaking type widening as the right shape for synthetic-row marker** — `representativeOwnerId: string` → `string | null` is non-breaking because `RecommendationCard.tsx` doesn't dereference the field. JSDoc on the type documents the synthetic-row convention. Synthetic rows route through the existing community-fallback rationale (no new copy surface). The whole sparse-pool fallback adds zero new UI templates.
- **10-day verification window for time-window rotation** — DISC-RECS-VARIATION specifically needs ≥6h between sessions to observe rotation. The natural 10-day delay between code-complete (2026-05-30) and milestone close (2026-06-09) covered the window with no operator effort beyond patience. `feedback_mobile_ui_verify_on_prod` patience pattern generalizes to algorithm-rotation verification.

### What Was Inefficient

- **6th consecutive `gsd milestone.complete` extractor garbage hit** at close — auto-generated accomplishments at close were `"1. [Rule 3 — Blocking issue] Plan's revalidateTag(tag) single-arg form is deprecated in Next 16 → migrated to updateTag(tag)"` (a fragment lifted from 75-01 SUMMARY's "What Changed From Plan" numbered list) and `"Files exist:"` (a section header from 75-02 SUMMARY). Hand-rewrote MILESTONES.md per the operator pattern documented in `project_v7_0_complete` / `project_v8_1_complete`. 6 milestones running (v6.0 / v7.0 / v8.0 / v8.1 / v8.2 — the v7.0 entry was hand-rewritten, the rest auto-generated then hand-corrected). The extractor heuristic is reliably broken; it should either be fixed or removed.
- **3rd recurrence of phase-dir archival miss** — `gsd milestone.complete` returned `"archived": {"phases": false}`. The CLI archives top-level docs (ROADMAP, REQUIREMENTS, MILESTONE-AUDIT if exists) but does NOT archive `.planning/phases/` directories. Per `feedback_milestone_close_phase_dir_archival_miss` memory: archive inline with `git mv .planning/phases/<phase>/ → .planning/milestones/v<X.Y>-phases/<phase>/` BEFORE the next `/gsd-new-milestone` runs `phases.clear --confirm`. Caught and resolved inline.
- **SEED-017 mis-classified as still-active at close** — even though Phase 75 shipped SEED-017's exact scope, the pre-close audit listed it under "Unimplemented Seeds." The audit doesn't know which seed a milestone implements; the operator does. Hand-patched the seed's frontmatter (`status: active` → `status: shipped` + `shipped_in: v8.2` + `shipped: 2026-06-09`) before close so it stops appearing as forward roadmap. Same pattern bit v6.0/v7.0/v8.0 closes (SEED-008/010/012/013/015/016 still listed as active despite being shipped). Worth a `seed-mark-shipped <id> <version>` CLI helper.
- **Audit signal-to-noise still poor (v8.2 audit = 27 items, all carried forward unchanged)** — same long-tail backlog as v8.1's 29 items minus SEED-017 (shipped) and minus the v8.1 UAT false-positive. The "acknowledge all" step continues to be fixed-cost ritual friction. The 6 already-shipped-but-not-flipped seeds are signal that should be one-click cleanable.
- **No `/gsd-audit-milestone` ran** — extends the audit-skip streak to 5 consecutive polish milestones (v5.0 / v5.1 / v7.0 / v8.0 / v8.1 / v8.2). For 2-req polish milestones, the audit cost feels disproportionate to risk — but the skip habituates. Worth considering whether the polish-milestone close ritual deserves a lighter `/gsd-audit-polish` that proves req-coverage in <5 minutes.

### Patterns Established

- **2-plan polish-milestone shape** — the smallest viable GSD cycle (smaller than v8.1's 3 phases). Wave-1-parallel only works when file overlap is zero; here cache-wiring (CollectorsLikeYou + watches.ts) and algorithm variation (recommendations.ts + discoveryTypes.ts) had clean separation. The shape is reusable for any polish milestone that targets exactly two orthogonal subsystems.
- **Code-complete vs milestone-close decoupling** — code-complete on 2026-05-30 (~2 hours), milestone close on 2026-06-09 (10 days later). STATE.md `status: verifying` correctly held the gap. The natural delay exercised the DISC-RECS-VARIATION ≥6h window without operator effort. Generalize: when a milestone has a time-bound verification step (rotation windows, daily cron refresh, cross-session checks), `status: verifying` can sit for days without drift if the working tree is clean.
- **Executor API-drift judgment as expected, not exception** — Plan 01's `revalidateTag(tag)` → `updateTag(tag)` swap was the right call. Future plans specifying read-your-own-write should call out `updateTag` directly. Document in MEMORY as `project_next16_revalidatetag_deprecated`. The same kind of judgment will keep being valuable as frameworks evolve faster than plans.

### Key Lessons

33. **(v8.2)** Next 16's `revalidateTag` signature drifted to `(tag, profile)`; the single-arg form is deprecated. For read-your-own-write Server Actions, use `updateTag(tag)` — the named Next 16 primitive. For cross-user stale-while-revalidate, keep `revalidateTag(tag, 'max')`. The plan-vs-implementation gap is small (same semantics, different name), but planners writing Next 16 cache code should default to the named primitive. Memory: `project_next16_revalidatetag_deprecated`.

34. **(v8.2)** The 6th consecutive `milestone.complete` extractor garbage confirms the heuristic is reliably broken across at least 5 milestone shapes (compound v6.0, 7-phase v7.0, 6-phase v8.0, 3-phase v8.1, 1-phase v8.2). Hand-rewriting at close is now part of the muscle memory but it shouldn't be. The extractor should either parse SUMMARY.md structure faithfully (look for `## What Built` / `## What Shipped` headers and extract their first paragraph each) or be removed in favor of operator-written accomplishments. Worth a SDK fix.

35. **(v8.2)** Audit-driven "Unimplemented Seeds" lists need a "this seed shipped in milestone X" signal. The operator-side correction (flip `status: active → shipped` + add `shipped_in:`) is reliable but easy to forget. A `seed-mark-shipped <id> <version>` CLI helper that updates the frontmatter atomically would close this loop. 6 already-shipped seeds are still mis-classified in the active backlog (SEED-008/010/012/013/015/016).

36. **(v8.2)** Polish milestones with operator-verified, time-bounded verification (rotation windows, cron-refresh dependencies) tolerate long code-complete → close gaps without working-tree drift, IF `status: verifying` is set and no parallel work touches the same files. v8.2 sat 10 days at `verifying` with zero drift; the natural delay exercised the DISC-RECS-VARIATION 6h window organically. Pattern applies to any milestone whose acceptance criteria require real-world time passage rather than active operator effort.

### Cost Observations

- Model mix: ~100% opus 4.7 (planning + execution; verification skipped — closed without `/gsd-audit-milestone`)
- Sessions: 2 (initial GSD chain on 2026-05-30 — plan-phase → execute-phase; close-only on 2026-06-09 — `/gsd-complete-milestone`)
- Notable: **smallest milestone by phase count since v1.0** (1 phase / 2 plans / 6 tasks / 14 commits). Lower bound for a viable GSD cycle. The 10-day code-complete → close gap is also the longest such gap (v8.1 was same-day; v8.0 was same-day; v7.0 was same-day). Polish-milestone overhead (close ritual, archival, MILESTONES.md rewrite, STATE.md hand-correction) consumed roughly the same time as the code work itself — a tax that's hard to justify on this scale of change. Consider whether 1-phase polish-milestones should bundle into a parent milestone (e.g., "v8.x Polish Pack") rather than ship as standalone closures.
...
| v8.2 | ~2 hours code + 10 days verify | 1/1 | Smallest milestone by phase count since v1.0 (1 phase / 2 plans). Wave-1-parallel split with zero file overlap (cache-wiring vs algorithm variation) — both verified independently. First project encounter with Next 16's `revalidateTag` signature drift; executor migrated 4 mutation sites + tests from spec'd single-arg `revalidateTag(tag)` to `updateTag(tag)` (named RYO primitive); durable as `project_next16_revalidatetag_deprecated`. Inline `mulberry32` 5-LOC PRNG seeded by `(viewerId, floor(Date.now() / 6h))` for cache-stable 6h-window rotation — same window = same shuffle = cache hit, next window rotates. Sparse-pool catalog-popularity top-up with non-breaking type widening (`representativeOwnerId: string | null`) routes synthetic rows through existing community-fallback rationale — zero new copy. Code-complete vs close decoupled by 10 days; `status: verifying` held the gap without drift; natural delay exercised the ≥6h rotation window organically. **6th consecutive recurrence** of `milestone.complete` extractor garbage; 3rd recurrence of phase-dir archival miss. Closed without `/gsd-audit-milestone` (extends the audit-skip streak to 6 milestones). 27 audit items acknowledged (carried from v8.1 unchanged minus SEED-017 shipped). Polish-milestone overhead (close ritual) roughly equaled code time — question of whether 1-phase polish-milestones should bundle into "v8.x Polish Pack" parent milestones opened |
...
| v8.2 | Build green; targeted vitest green on 2 new test files (228 + 413 LOC, +874/−11 LOC across 6 files); zero existing-test churn; no new static guards added | Not configured | All v8.1 carryover (DEBT-12 cleared earlier; 31 v3.0 + Phase 35/41 UAT still open; long-tail 11 quick-task backlog from April-May 2026 still open) carried unchanged into v8.2 deferred items; SEED-017 cleared (shipped THIS milestone); 6 already-shipped seeds (SEED-008/010/012/013/015/016) still mis-classified as `active/dormant` — durable backlog-housekeeping debt that's hit 4 consecutive milestones now (v7.0/v8.0/v8.1/v8.2); audit-skip streak now 6 milestones (v5.0/v5.1/v7.0/v8.0/v8.1/v8.2); REQUIREMENTS.md 2/2 `[x]` traceability filled (manual fix at close — frontmatter checkboxes were `[x]` but traceability rows said "Not started"; pattern: REQUIREMENTS.md has two sources of truth for the same fact and they drift) |
