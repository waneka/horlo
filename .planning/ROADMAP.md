...
<details>
<summary>✅ v8.2 Discovery Freshness (Phase 75) — SHIPPED 2026-06-10</summary>

- [x] Phase 75: Recommendations Freshness — Cache Invalidation + Algorithm Variation (2/2 plans) — DISC-RECS-CACHE, DISC-RECS-VARIATION

2/2 v8.2 requirements shipped. Single-phase polish milestone; ~2 hours code work on 2026-05-30, close held open until 2026-06-09 (10-day gap exercised the DISC-RECS-VARIATION ≥6h rotation window comfortably). 14 commits since v8.1 close; +874 / −11 LOC across 6 src/+tests/ files. Closed without `/gsd-audit-milestone` (mirrors v5.0/v5.1/v7.0/v8.0/v8.1 polish-milestone close pattern).

Notable: first project encounter with Next 16's `revalidateTag` signature drift — executor migrated all 4 mutation sites from plan-spec'd single-arg `revalidateTag(tag)` to `updateTag(tag)` (the named Next 16 read-your-own-write primitive). Lesson durable as `project_next16_revalidatetag_deprecated`.
...
See [v8.3-ROADMAP.md](milestones/v8.3-ROADMAP.md) for full phase details.
...
**Plans**: 4 plans (Wave 1 parallel — all three touch disjoint files; 83-04 gap closure)
...
- [x] 85-06-PLAN.md — Wave 4: owner-only previously-owned data split + "Show previously owned" toggle + empty-state gate; visitor Notes exclusion
...
- [ ] 85-09-PLAN.md — Wave 5: canvas-confetti legitimacy checkpoint + celebratePromotion wired into AddWatchFlow and WatchForm (LIFE-04)
...
| 85. Collection lifecycle | v9.0 | 7/11 | In Progress|  |
