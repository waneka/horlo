...
- [x] **UI-03**: `WatchForm` (edit existing watch) renders the canonical `brands.name` / `watch_families.name` resolved from `catalogId` as read-only display strings (with an "Edit catalog mapping" admin link visible only to the watch owner) — user cannot edit brand/model into a non-canonical string after v8.4.
