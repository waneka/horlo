# Requirements Archive: v8.0 v8.0
...
- [x] **CLNP-01**: `VerdictStep.tsx`, `WishlistRationalePanel.tsx`, and `PasteSection.tsx` are deleted along with their test files; no callers remain in the codebase
- [x] **CLNP-02**: Static guard `tests/static/AddWatchFlow.no-verdict-step.test.ts` (`// @vitest-environment node`) fails the build if any of the three deleted components reappear in `AddWatchFlow.tsx`
- [x] **CLNP-03**: Static guard `tests/static/AddWatchFlow.no-collection-fit-card.test.ts` (`// @vitest-environment node`) mirrors the Phase 20 `tests/static/CollectionFitCard.no-engine.test.ts` pattern — fails CI if `CollectionFitCard` is imported by any file in the add flow
- [x] **CLNP-04**: `RecentlyEvaluatedRail` is removed from `AddWatchFlow`; component file disposition (delete vs. retain for future repurpose) decided during plan-phase
...
| CLNP-01 | Phase 71 | Complete |
| CLNP-02 | Phase 71 | Complete |
| CLNP-03 | Phase 71 | Complete |
| CLNP-04 | Phase 71 | Complete |
