...
**Active:** v4.0 Discovery & Polish — Phase 19.1 Catalog Taste Enrichment complete (2026-04-29). Architectural insertion before Phase 20: 8 LLM-derived taste columns (formality, sportiness, heritage_score, primary_archetype, era_signal, design_motifs, confidence, extracted_from_photo) added to `watches_catalog` with 3 CHECK constraints; `catalog-source-photos` Supabase Storage bucket provisioned with 4 folder-enforced RLS policies. New `src/lib/taste/` service module wraps Anthropic claude-sonnet-4-6 with strict tool-use + closed vocabularies. `updateCatalogTaste` DAL helper enforces first-write-wins (D-13) via conditional `AND confidence IS NULL` predicate; `applyUserUploadedPhoto` writes user-uploaded reference photos through to canonical `image_url` (D-21). `addWatch` Server Action and `/api/extract-watch` route both dispatch enrichment fire-and-forget after catalog upsert; `extractWithLlm()` body byte-identical (D-07 lock confirmed). `WatchForm` strips Style/Role/Design pickers (D-18) and gains a Reference Photo Card on `mode='create'` only (D-19) backed by new `CatalogPhotoUploader` component. Operator scripts: `scripts/backfill-taste.ts` (post-deploy gate, dry-run default cost estimate) and `scripts/reenrich-taste.ts` (recovery, --force-required safety guard). 22/22 D-XX decisions verified against the live codebase; 31 new unit/component tests + 26 live-DB integration tests + 4 API tests + 2 backfill tests green. 3 non-blocking warnings in 19.1-REVIEW.md.
...
---
*Last updated: 2026-04-29 — Phase 19.1 (Catalog Taste Enrichment) complete. v4.0 architectural insertion before Phase 20: 8 LLM-derived taste columns added to `watches_catalog`, `catalog-source-photos` Supabase Storage bucket with 4 folder-enforced RLS policies, `src/lib/taste/` service module (Anthropic claude-sonnet-4-6 strict tool-use + closed vocabularies), `updateCatalogTaste` DAL helper with first-write-wins (D-13), `applyUserUploadedPhoto` photo write-through (D-21), fire-and-forget enrichment wired into `addWatch` + `/api/extract-watch` (D-09; `extractWithLlm()` body byte-identical per D-07 lock), `WatchForm` surgery (3 picker Cards hidden + Reference Photo Card on create-mode only via new `CatalogPhotoUploader`), and operator tooling (`scripts/backfill-taste.ts` + `scripts/reenrich-taste.ts` + runbook). 22/22 D-XX decisions verified; 31 unit/component + 26 live-DB integration + 4 API + 2 backfill tests green; 3 non-blocking warnings in 19.1-REVIEW.md. Up next: Phase 20 (Collection Fit + Verdict Copy + Cross-User Surface Polish).*
