...
---
*Last updated: 2026-04-22 — Phase 12 complete. Visibility ripple landed across the DAL: `getWearEventsForViewer` (three-tier owner bypass + per-row follow check), `getWearRailForViewer` leftJoin(follows) with per-row visibility, feed hot path gates on `activities.metadata->>'visibility'` (fail-closed on NULL), wishlist action uses three-tier visibility + follows with Letterboxd-style uniform "not found". Worn History settings toggle retired. `worn_public` column dropped from schema + local + prod (via `supabase db push --linked --include-all`). Phase 11 migrations 6/7 were renamed to 46/47 and the SECDEF fix (formerly 004b) to 045 so lexical ordering puts function-creation before grant-revoke. pg_trgm migration schema-qualified `extensions.gin_trgm_ops` for prod push. Test suite 2105+ passed after gap closure plan 12-07 rewrote `wishlist.test.ts` mocks to the three-tier contract (Tests 10/11 added for followers happy/deny). Three HUMAN-UAT items (prod schema confirmation + 2 browser smokes) user-approved at phase close. Up next: Phase 999.1 (Phase-5 code review follow-ups / RLS errors) in the v3.0 backlog.*
