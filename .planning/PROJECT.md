...
---
*Last updated: 2026-04-25 — Phase 16 (People Search) complete. `/search` shipped with 4 tabs (All / Watches / People / Collections); People populated with debounced live results, taste overlap pill, and inline FollowButton; other three tabs render "coming soon" cards with no query firing. `searchProfiles` DAL uses `pg_trgm` ILIKE on `username` + bio search gated at `q.length>=3` with two-layer privacy and pre-LIMIT 50 + JS sort + batched `isFollowing` (Pitfall C-4). `searchPeopleAction` Server Action gates auth + Zod `.strict().max(200)`. `useSearchState` hook (250 ms debounce + AbortController + URL sync + 2-char client minimum). XSS-safe `HighlightedText` (no `dangerouslySetInnerHTML`). Persistent nav search input restyled (D-24); `HeaderNav` deleted (D-23) — Profile/Settings reachable only via UserMenu dropdown. Pitfall C-1 (pg_trgm GIN index reachability) verified via forced-plan EXPLAIN ANALYZE evidence; UAT approved. 2813 tests pass / 0 regressions. Code review: 0 critical, 2 warnings, 5 info (non-blocking, tracked in 16-REVIEW.md). v3.0 milestone complete — all 6 phases shipped. Up next: complete-milestone or new-milestone.*
