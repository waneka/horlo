...
**Active:** v4.0 Discovery & Polish — Phase 19 /search Watches + Collections complete (2026-04-28). The two stub tabs from v3.0 Phase 16 (`?tab=watches` and `?tab=collections`) are now populated with real, debounced, anti-N+1, two-layer-privacy results. `searchCatalogWatches` DAL hydrates Owned/Wishlist viewer state via single batched `inArray` keyed by `viewerId` (SRCH-09 + SRCH-10); `searchCollections` DAL ANDs `profile_public = true` AND `collection_public = true` AND `id != viewerId` (SRCH-11 + SRCH-12) — locked by live-DB integration test. Hook surface `useSearchState` extended with three independent sub-effects (one per People/Watches/Collections section), each owning its own `AbortController` per RESEARCH Q4 path A — per-section paint independence + per-section abort granularity (SRCH-13 + SRCH-14). `AllTabResults` composer applies defense-in-depth `slice(0, 5)` inside the renderer (I-2 BLOCKER fix) so the UI never over-caps even if the hook's invariant breaks. `SearchPageClient` rewritten to consume per-tab DAL slices directly, replacing the v3.0 `<ComingSoonCard>` panels. 7/7 SRCH requirements satisfied at code level; 116 unit + RTL tests green + 2 live-DB integration tests green; 8 advisory findings in 19-REVIEW.md (3 warning + 5 info, all sub-Critical and non-blocking).
