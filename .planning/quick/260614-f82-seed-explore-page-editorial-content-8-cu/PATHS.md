...
rationale: |
  ...
  step adds something the previous could not — better movement, deeper rating,
  richer heritage — without abandoning the original conviction.
nodes:
  - brand: Tudor
    model: Black Bay 58
    reference: 79030N
...
rationale: |
  ...
  from the contemporary hand-wound reference to the original flight-ready cases
  that preceded the moon missions.
nodes:
  - brand: Omega
...
rationale: |
  ...
  same wrist presence and legibility demands, completely different design
  philosophies. The first branch goes tool-purist; the second goes dressy;
  the third goes vintage-inspired. All three serve the same collector who
  wants to understand what else their wrist could be wearing.
nodes:
  - brand: Tudor
    model: Pelagos 39
    reference: 25407N
...
rationale: |
  ...
  heritage toward three different chronograph traditions: field use, German
  precision, and the sport of horology for its own sake.
nodes:
  - brand: Tudor
    ...
    sort_order: 0
  - brand: Hanhart
    model: 417 ES Flyback Panda, 42 mm
    reference: H721.201-
    rationale: German flyback chronograph with military pilot heritage — branches toward the technical and the purposeful.
    sort_order: 1
  - brand: Rolex
    model: Cosmograph Daytona
    reference: 126500LN
...
rationale: |
  ...
  adds one dimension the 5 Sports could not offer while keeping the fundamental
  automatic sport-watch character that made the entry point worth buying.
nodes:
  - brand: Seiko
    model: Alpinist
    reference: SPB121
...
seed:
  ...
  reference: L3.781.4.06.6
rationale: |
  Longines HydroConquest is a genuine Swiss-made automatic diver at an
  accessible price — a real watch, not a stepping stone. But for collectors
  who want to understand where the extra money in Swiss horology goes, this
  path follows the logical upgrade ladder while staying in the sport-watch lane.
nodes:
  - brand: Tudor
    model: Black Bay 58
    reference: 79030N
...
## filling-a-gap-1
slug: filling-a-gap-1
path_type: Filling a Gap
sort_order: 70
seed:
  brand: Rolex
  model: Submariner No-Date
...
rationale: |
  ...
  legible field watch for casual wear. This path fills the obvious holes
  — each step adds a role the Submariner cannot play.
nodes:
  - brand: Omega
    model: Constellation Globemaster
    reference: 130.33.39.21.02.001
    rationale: Annual calendar dress watch for the occasions the Sub overdresses — a genuine role the diver cannot fill.
    sort_order: 0
  - brand: Seiko
    model: Alpinist
    reference: SARB017
...
rationale: |
  ...
  a field beater for weekends, a chronograph for events, and a GMT for the
  collector whose Snowflake has made them want to travel more.
nodes:
  - brand: Seiko
