...
import type { Watch, WatchEditResult, MarkPreviouslyOwnedInput } from '@/lib/types'
...
// Phase 85 (LIFE-03) — disposal dialog commit payload. `.strict()` rejects
// extra keys (e.g. `status`) — a mass-assignment guard (T-85-10): status is
// always set server-side to 'previously_owned', never accepted from the
// client. `today` is required (not optional) — this action has no other
// source of "today" and 260622-exo forbids deriving it server-side.
const markWatchPreviouslyOwnedSchema = z
  .object({
    watchId: z.string().uuid(),
    disposalReason: z.enum(['sold', 'lost', 'gifted', 'stolen', 'traded']),
    sellPrice: z.number().min(0).optional(),
    disposalDate: isoCalendarDate.optional(),
...
/**
 * Mark an owned watch as previously owned (LIFE-03 disposal commit).
 *
 * The disposal dialog's ONLY commit path — a dedicated action rather than a
 * route through `editWatch`, so the "reason required + only from owned"
 * contract stays explicit and `editWatch` can hard-reject transitions INTO
 * previously_owned (D-07). Sequence: auth -> zod (.strict(), T-85-10) ->
...
export async function markWatchPreviouslyOwned(
  ...
  const parsed = markWatchPreviouslyOwnedSchema.safeParse(input)
  if (!parsed.success) {
    return { success: false, error: 'Invalid request' }
  }

  // T-85-09: bound the client-supplied `today` to what some real timezone
  // could currently be (±14h of server UTC) before trusting it for the
  // disposalDate comparison below. The server never derives "today" itself
  // (260622-exo).
  if (!isPlausibleClientToday(parsed.data.today)) {
    return { success: false, error: 'Invalid request' }
  }

  // D-06: lexical comparison of ISO YYYY-MM-DD strings is chronological.
  if (parsed.data.disposalDate && parsed.data.disposalDate > parsed.data.today) {
    return { success: false, error: "Disposal date can't be in the future." }
  }

  try {
    // T-85-08: second IDOR layer — scoped to the caller's own watches.
    const priorRow = await watchDAL.getWatchById(user.id, parsed.data.watchId)
    if (!priorRow) {
      return { success: false, error: 'Watch not found' }
    }

    // T-85-13: double-submit idempotency — no-op, no re-write.
    if (priorRow.status === 'previously_owned') {
    ...
    if (priorRow.status !== 'owned') {
      return {
        success: false,
        error: `Cannot mark ${priorRow.status} watch as previously owned`,
      }
    }
    ...
    const watch = await watchDAL.updateWatch(user.id, parsed.data.watchId, {
    ...
    // D-08 cache fan-out — matches the invalidation matrix every other watch
    // mutation in this file uses, EXCEPT the profile tag: this uses
    // updateTag (read-your-own-writes, the Phase 84 logBackfillWear
    // precedent) instead of revalidateTag(tag, 'max') so the card leaves the
    // Collection grid immediately for the owner, who stays on the tab.
    revalidatePath('/')
    updateTag(`viewer:${user.id}:recs`)
    revalidatePath('/u/[username]', 'layout')
    const ownerProfile = await getProfileById(user.id)
    ...
    console.error('[markWatchPreviouslyOwned] unexpected error:', err)
    if (err instanceof Error && err.message.includes('not found or access denied')) {
      return { success: false, error: 'Not found' }
    }
    return { success: false, error: "Couldn't update this watch." }
  }
}
