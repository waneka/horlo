...
/**
 * InsightsTabContent — lifted from the retired /insights route (D-13).
 *
 * Owner-only content. Gated upstream by [tab]/page.tsx: this component is
 * only invoked when viewer.id === profile.id. The gate guarantees
 * profileUserId equals the viewer's id at call time, so all DAL reads are
 * self-scoped. Two-layer privacy (P-08 / Phase 12 pattern): tab strip
 * omission + [tab]/page.tsx notFound() mean non-owners never reach this
 * renderer.
 */
...
export async function InsightsTabContent({
  profileUserId,
  ...
  // Safe to pass profileUserId here: this component is only invoked when
  // viewer.id === profile.id (owner gate in [tab]/page.tsx).
  const watchIds = ownedWatches.map((w) => w.id)
  const wearDateMap = await getMostRecentWearDates(profileUserId, watchIds)
  const ownedWithWear: WatchWithWear[] = ownedWatches.map((w) => ({
    ...w,
    lastWornDate: wearDateMap.get(w.id) ?? undefined,
  }))

  const styleDistribution = calculateDistribution(
    ownedWatches,
    (w) => w.styleTags,
  )
  const roleDistribution = calculateDistribution(
    ownedWatches,
    (w) => w.roleTags,
  )
